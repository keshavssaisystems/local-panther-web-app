...
export function Department({ showValidation, validationMessage, mandatory }) {
    ...
    return (
        <>
           <SelectFormGroup label="Department" id="department" name="department" defaultOption="Select Department" optionData={departmentOptions.length > 0 ? departmentOptions : []} showValidation={showValidation} validationMessage={validationMessage} mandatory={mandatory} />              
        </>
    );
}