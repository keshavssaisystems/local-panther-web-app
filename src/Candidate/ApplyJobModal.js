...
export function ApplyJobModal({jobId}) {
    ...
    const [showNotivePeriodValidation, setNoticePeriodValidation] = useState(false);
    const toggle = () => {
        setModal(!modal);
    ...
    const checkValidation = (event) => {
        ...
        if(event.target.elements.experience.value !== "" && event.target.elements.noticePeriod.value !== ""){
            onApplyClick(event);
        }
    }
            ...
            <Modal isOpen={modal} toggle={toggle} fullscreen={"md"} size="lg" backdrop={"static"}>
                ...
                <ModalBody>
                    <Form onSubmit={checkValidation}>
                        <InputFormGroup label={"Years of Experience"} name={"experience"} id={"experience"} type={"number"} placeholder={"Enter years of experience"} showValidation={showExperienceValidation} validationMessage={"Please Enter Years of Experience"} />
                        <NoticePeriod showValidation={showNotivePeriodValidation} validationMessage={"Please Select Notice Period"}/>
                        <CheckboxFormGroup id="relocate" name="relocate" label="Are you willing to relocate?" />
                        <CheckboxFormGroup id="isvideoconference" name="isvideoconference" label="Will you be available for video conference" />
                        <Button color="primary" type="submit" className="float-end"> Apply </Button> 
                        <Button color="secondary" onClick={toggle}> Cancel </Button>
                    </Form>
                </ModalBody>
            </Modal>
            <SweetAlert title="Applied Successfully"  show={showSuccess} type="success" onConfirm={() => setShowSuccess(false)}/>
        </>
    );
}